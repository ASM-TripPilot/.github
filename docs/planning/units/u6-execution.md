...
- 어댑터 타임아웃·서킷·격자 변환은 앱 소관(Resilience4j·LCC DFS — U6-W1) — 인프라는 SI §8.2 A12(어댑터 실패율·서킷 open)·A11(기상청 쿼터 80%) 알람을 제공(§7.6). TMap 도로 거리(재계획 이동시간)는 C2 소유(U5) — U6 미호출, 실패 시 직선거리×우회계수(G106) 폴백.
