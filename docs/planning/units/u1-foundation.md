...
| PD-4 | 중앙 로그·모니터링·알림 | **CloudWatch Logs/Metrics/Alarms + SNS**, APM·크래시=Sentry |
...
| Configuration Set | `trippilot-{env}-transactional` — 바운스·컴플레인·딜리버리 이벤트 → CloudWatch 메트릭 |
...
전부 CloudWatch Logs 메트릭 필터(구조화 JSON 보안 이벤트가 소스) → 알람 → SNS `trippilot-{env}-alerts`.
