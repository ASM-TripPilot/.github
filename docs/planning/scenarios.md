...
앱은 자체 예약 시스템·인앱 결제·내부 객실 재고를 두지 않는다. 공급은 외부 OTA 제휴 링크로 위임하고, 앱은 **일정을 짜고, 지키고, 남기는** 데 집중한다. 초기 출시는 국내(대한민국)에 한정하며 지도는 국내 API(카카오·네이버·TMap)를 우선 채택한다.

---

## 2. 여정의 두 진입 경로 — Case A와 Case B

여행자가 계획을 시작하는 방식은 둘로 갈린다. TripPilot은 **두 경로를 모두 1급(first-class)으로 지원**하되, 어느 쪽으로 들어와도 같은 지점 — **등록된 숙소 = 일정의 출발점** — 으로 수렴시킨다(ADR-0002의 '예약=출발점'을 '등록된 숙소=출발점'으로 계승).

| | Case B — 숙소 우선 | Case A — 장소 우선 |
|---|---|---|
| 성향 | 숙소를 먼저 정하고 그 위치에 맞춰 갈 곳을 짠다 | 가고 싶은 곳을 먼저 모으고 그 동선에 맞는 숙소를 잡는다 |
| 시작 행동 | 외부 OTA 예약/앱 저장 → 숙소 '등록' | 가고 싶은 POI 저장 → 위시리스트 |
| 시스템 처리 | 숙소의 위치·체크인/아웃 날짜가 일정 생성의 기준점(US-E3-06) | 저장 POI가 여행 생성 시 '꼭 갈 곳'(필수 방문지)의 시드로 투입(US-E2-05) |
| 수렴점 | 등록 숙소 기준 AI 일정 생성 | 숙소를 직접 탐색·등록 → 같은 수렴점. 저장 POI 집합으로 숙소 '권역'을 역추천하지는 않는다 |

세 번째 변형으로 **숙소 나중 등록**이 있다 — 일정을 먼저 완성한 뒤, 완성 동선(무게중심)을 기준으로 한 숙소 권역 추천을 제공한다(US-E5-11). 이 경우에만 동선→숙소 방향의 추천이 열린다.

```mermaid
flowchart TD
    B["Case B 숙소 우선<br/>OTA 예약·앱 저장 → 숙소 등록"] --> C["등록된 숙소 = 일정의 출발점<br/>위치 + 체크인/아웃 날짜"]
    A["Case A 장소 우선<br/>POI 저장 → '꼭 갈 곳' 시드"] --> C
    A2["변형: 숙소 나중 등록<br/>동선 먼저 → 숙소 권역 추천"] --> C
    C --> D["AI 일정 생성 · E5 · U5"]
```
...
| 탐색·생성 | 숙소 없이 여행지·날짜만으로 여행을 만들고 '직접 만들기' 또는 '완전 AI'로 뼈대만 잡음 | 최소 입력 생성 허용, 등록 숙소 없으면 생성 버튼 비활성+등록 유도(단, 숙소 나중 온램프는 예외 — US-E5-01) |
...
| — | 숙소 없이 AI 일정 생성 시도 | 등록 숙소 0개면 일반 생성 버튼 비활성. 단 '숙소 나중 등록' 온램프는 예외로 허용 | US-E5-01·US-E5-11 |
...
- **생성 기준**: 등록 숙소 위경도·체크인/아웃을 고정 입력값으로 날짜별 일정 1개씩. 생성·재계획은 서버 연산(D28). 숙소 없으면 버튼 비활성(숙소 나중 온램프는 예외).
