...
TripPilot(RN Expo + Spring Boot Kotlin 모듈러 모놀리스 + PostgreSQL 기반 B2C 여행자 슈퍼앱)의 개발 기획 정본이다. AI-DLC 산출물을 팀이 그대로 읽을 수 있게 재구성한 문서 모음이며, 원본을 열지 않아도 이 폴더만으로 제품·설계·개발 순서를 완전히 파악할 수 있다.
