...
| 진입 경로 | 정의 | 동작 |
|---|---|---|
| **숙소 우선(Case B)** | 숙소를 먼저 정하고 그 위치에 맞춰 갈 곳을 짠다 (숙소 → 일정) | 외부 OTA에서 예약했거나 앱에서 저장한 숙소를 '등록'하면, 그 숙소의 **위치와 체크인/아웃 날짜**가 AI 일정 생성의 **공간 앵커**가 된다. 아직 예약을 확정하지 않은 사용자에게는 숙소 *후보*별로 "이 숙소에 묵으면 당신 취향의 장소들이 얼마나 가까운지 · 어느 후보가 더 가까운지" 식의 **일정 미리보기**를 제시해, 위치를 잘못 골라 동선이 묶이는 후회를 예약 전에 줄인다. (ADR-0002의 '예약=출발점' 원칙을 '등록된 숙소=공간 앵커'로 계승) |
| **장소 우선(Case A)** | 가고 싶은 곳을 먼저 모아 일정을 짜고, 그 동선에 맞는 숙소를 잡는다 (장소 → 일정 → 숙소) | 가고 싶은 장소(POI)를 저장하면 여행 생성 시 **'꼭 갈 곳'(필수 방문지) 시드**로 투입된다. 숙소가 아직 없어도 **여행지 중심을 임시 앵커로 무숙소 초안 일정**을 만들고, 그 **완성 동선(무게중심)으로 숙소 권역을 추천**(US-E5-11)한다. 추천 권역에서 숙소를 등록하면 그 숙소를 앵커로 **재정렬**되어 숙소 우선과 같은 수렴점에 도달한다. (ADR-0002/0004 — 숙소는 최종 공간 앵커, 확정 시점만 경로마다 다름) |
...
| 지도·장소 | 카카오(장소 검색·지오코딩) + 카카오모빌리티(도로 거리) + 네이버(2차 폴백) | D08 |
