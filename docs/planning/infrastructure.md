...
**착수 전 신청 리드타임(코드보다 먼저):** 링크프라이스 어필리에이트+리워드 API = 1주+ (수동 승인 광고주), 기상청 활용신청 = 1일+, 카카오 개발자 앱+카카오맵 권한 = 즉시~. 제휴 승인과 코드는 병렬(fake provider로 로직 선검증). → 백엔드 착수 체크리스트는 `trippilot-backend/docs/design/개발착수전-체크리스트.md`.
